...
impl Document {
	...
	pub fn get_or_create_resources(&mut self, page_id: ObjectId) -> ObjectId {
		let obj_id = self.add_object(Dictionary::new());
		let mut page = self.get_object_mut(page_id).and_then(|obj| obj.as_dict_mut()).unwrap();
		// let resources_id = if page.has("Resources") {
		// 	page.get("Resources").and_then(|obj| obj.as_reference()).unwrap()
		// } else {
		page.set("Resources", Object::Reference(obj_id));
		obj_id
		// };
		// resources_id
	}
	...
	pub fn add_proc(&mut self, page_id: ObjectId, proc_name: Vec<u8>) {
		let resources_ids = self.get_page_resources(page_id);
		let resources_id = resources_ids[0];
		let mut resources = self.get_object_mut(resources_id).and_then(|obj| obj.as_dict_mut()).unwrap();
		...
		let mut proc_set = resources.get_mut("ProcSet").and_then(|obj| obj.as_array_mut()).unwrap();
		proc_set.push(Object::Name(proc_name));
	}
}
